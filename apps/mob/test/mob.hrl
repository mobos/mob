...
should_find_the_correct_node_and_deploy_a_service_test() ->
    ...
    meck:expect(discovery, where_deployed, fun(_) -> {error, not_found} end),
    meck:expect(discovery, find_available_node, fun(_) -> {ok, FakeNode} end),
    ...
    ?assertEqual(1, meck:num_calls(discovery, where_deployed, [ParsedService#service.name])),
    ?assertEqual(1, meck:num_calls(discovery, find_available_node, [ParsedService])),
...
should_not_deploy_an_already_deployed_service_test() ->
    ...
    meck:expect(service_parser, parse, fun(_) -> {ok, ParsedService} end),
    meck:expect(discovery, where_deployed, fun(_) -> {found, FakeNode} end),
...
should_ask_if_a_service_is_remotely_started_test() ->
    ...
    ?assertEqual(1, meck:num_calls(discovery, where_deployed, [ServiceName])),
...
should_add_a_child_a_to_a_remotely_spawned_parent_test() ->
    ...
    meck:expect(discovery, where_deployed, fun(_ServiceName) -> {found, FakeNode} end),
    ...
    ?assertEqual(1, meck:num_calls(discovery, where_deployed, [Parent])),
